...
namespace pcl
{
	bool computeB2BTL_MEAMPairFeature(const Eigen::Vector4f &p1, const Eigen::Vector4f &n1,
	...
	public:
		...
		B2BTL_MEAMHashMapSearch(float angle_discretization_step = 12.0f / 180.0f * static_cast<float>(M_PI),
			...
			: alpha_m_EAM(), feature_hash_map_(new FeatureHashMapType), encoded_hash_map_(new EncodedHashMapType), internals_initialized_(false), angle_discretization_step_(angle_discretization_step), distance_discretization_step_(distance_discretization_step), max_dist_(-1.0f), Lvoxel(0.01f)
		{
		}
		 ...
		 */
		void setInputFeatureCloud(PointCloud<PPFSignature>::ConstPtr feature_cloud, PointCloud<PointXYZ>::ConstPtr cloud_XYZ);
		...
		Ptr makeShared() { return Ptr(new B2BTL_MEAMHashMapSearch(*this)); }
		...
		inline float getAngleDiscretizationStep() { return angle_discretization_step_; }
		...
		inline float getDistanceDiscretizationStep() { return distance_discretization_step_; }
